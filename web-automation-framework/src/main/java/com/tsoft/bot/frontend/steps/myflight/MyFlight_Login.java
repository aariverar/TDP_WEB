...
public class MyFlight_Login {
    ...
    @Then("^se da clic en el boton Sing-IN no permitiendo ingresar correctamente$")
    public void seDaClicEnElBotonSingINNoPermitiendoIngresarCorrectamente() throws Exception {
        try {
            driver.findElement(By.name(BTN_SIGNIN)).click();
            ExtentReportUtil.INSTANCE.stepPass(driver, "Se dió clic en el botón SignIN");
            generateWord.sendText("Se dió clic en el botón SignIN ");
            ...
            ExtentReportUtil.INSTANCE.stepPass(driver, "Se dió clic en el botón SUBMIT");
            generateWord.sendText("Se dió clic en el botón SUBMIT");
            generateWord.addImageToWord(driver);
        }catch (Exception e){
            ExcelReader.writeCellValue(EXCEL_WEB, LOGIN_WEB, 1, 19, "FAIL");
            ExtentReportUtil.INSTANCE.stepFail(driver, "Fallo el caso de prueba : " + e.getMessage());
